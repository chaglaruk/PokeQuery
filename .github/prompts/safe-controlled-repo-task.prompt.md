...
1. Verify repository identity, current branch, HEAD, and worktree status against the expected task context.
2. Stop and request confirmation if the repository, branch, explicit HEAD baseline, or task scope is unexpected, or if unapproved unrelated worktree changes are present. Explicitly acknowledged pre-existing changes may remain untouched.
3. Read the authoritative repository instructions before editing.
4. Inspect which external tools are available and decide which ones are actually relevant.
5. Use the minimum relevant tools only; do not claim a tool was used unless a successful result was received.
6. Make the smallest approved patch that addresses the task.
7. Run the repository-defined local verification that best matches the touched files.
8. Use external validation only when the task justifies it, and keep that separate from local tests in the final report.
9. Review the final diff and check it against the repo safety/privacy rules.
10. Report assumptions instead of inventing repository behavior or policy.
