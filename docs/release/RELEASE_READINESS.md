...
The previous v0.7.4 AAB was not available locally for a direct certificate-fingerprint comparison. The v0.7.5 build used the existing configured Play upload keystore and its signer certificate was verified with `jarsigner`. Google Play will independently enforce upload-certificate continuity when the AAB is uploaded.
