...
export function SearchAssistantScreen() {
  ...
  const localizedLintMessage = useCallback((warning: LintWarning) => {
    ...
    return warning.isError ? t('search_assistant_lint_issue') : t('search_assistant_review_warning')
  }, [locale, t])
  ...
  return (
    <div className="page content-with-nav">
      ...
      <div className="card" style={{ marginBottom: '16px' }}>
        ...
        />
        <button className="btn btn-primary" onClick={handleParse} disabled={!input.trim()}>{t('search_assistant_parse')}</button>
      </div>
      ...
      {result && result.canBuild && (
        <div className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
            <p style={{ fontSize: '13px', fontWeight: 700, color: 'var(--accent)', margin: 0 }}>{t('search_assistant_suggested_string')}</p>
            {explained && <span className={`badge ${precisionBadgeClass(explained.precision)}`}>{t(precisionLabelKeys[explained.precision] ?? 'explain_prec_unknown')}</span>}
          </div>
          <div className="search-string" style={{ marginBottom: '12px' }}>{translatedQuery || result.rawQuery}</div>

          {resolvedLanguage !== 'English' && (
            <p style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '8px' }}>
              {t('search_assistant_locale_label', resolvedLanguage)}
              {unverifiedTokens.length > 0 && <>{' '}{t('search_assistant_unverified_tokens', unverifiedTokens.join(', '))}</>}
            </p>
          )}
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: '0 0 6px 0' }}>
                {warnings.map((w, i) => (
                  <li key={i} style={{ fontSize: '12px', lineHeight: 1.5, marginBottom: '4px', display: 'flex', gap: '6px', alignItems: 'flex-start' }}>
                    <span className={w.isError ? 'lint-marker-error' : 'lint-marker-advisory'} aria-hidden="true"><AppIcon name={w.isError ? 'error' : 'warning'} size={16} /></span>
                    <span>{localizedLintMessage(w)}</span>
                  </li>
                ))}
              </ul>
              {copyBlocked && <p style={{ fontSize: '12px', fontWeight: 700, color: 'var(--danger)' }}>{t('search_intent_fix_errors')}</p>}
              {hasAdvisory && <p style={{ fontSize: '12px', fontWeight: 700, color: 'var(--warning)' }}>{t('search_intent_lint_advisory')}</p>}
            </div>
          )}
      ...
      {result && result.canBuild && (
        <div className="card">
          {result.tokens.length > 0 && (
            <>
              <p style={{ fontSize: '13px', fontWeight: 700, color: 'var(--accent)' }}>{t('search_assistant_tokens')}</p>
              <div style={{ fontFamily: 'SF Mono, Consolas, monospace', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '8px', padding: '10px 12px', marginBottom: '12px' }}>
                {result.tokens.map((token, i) => <div key={i} style={{ fontSize: '13px', color: 'var(--text)' }}>+ {token}</div>)}
                {result.exclusions.map((ex, i) => <div key={`ex-${i}`} style={{ fontSize: '13px', color: 'var(--warning)' }}>! {ex} ({t('search_assistant_excluded')})</div>)}
              </div>
            </>
          )}
          ...
          {result.explanationKey && (
            <>
              <p style={{ fontSize: '13px', fontWeight: 700, color: 'var(--accent)', marginBottom: '4px' }}>{t('search_assistant_explanation')}</p>
              <p style={{ fontSize: '12px', color: 'var(--text-dim)', lineHeight: 1.6, marginBottom: '12px' }}>
                {t(result.explanationKey) !== result.explanationKey ? t(result.explanationKey) : result.explanation}
              </p>
            </>
          )}
          ...
          {result.limitationKeys.length > 0 && (
            <>
              <p style={{ fontSize: '13px', fontWeight: 700, color: 'var(--warning)', marginBottom: '4px' }}>{t('search_assistant_limitations')}</p>
              <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                {result.limitationKeys.map((key, i) => {
                  const txt = t(key) !== key ? t(key) : (result.limitations[i] ?? key)
                  return <li key={i} style={{ fontSize: '12px', lineHeight: 1.5, marginBottom: '4px', display: 'flex', gap: '6px' }}><span style={{ color: 'var(--danger)' }} aria-hidden="true">{'\u2022'}</span><span style={{ color: 'var(--text-dim)' }}>{txt}</span></li>
                })}
              </ul>
            </>
          )}
          ...
          {explained && (
            <div style={{ marginTop: '12px' }}>
              <span className={`badge ${riskBadgeClass(explained.totalRisk)}`} style={{ marginRight: '8px' }}>{t(riskDisplayKey(explained.totalRisk))}</span>
              <span style={{ fontSize: '11px', color: 'var(--text-dim)' }}>{t('search_assistant_scope')}: {t(scopeBreadthKeys[explained.scopeBreadth] ?? 'explainer_scope_moderate')}</span>
            </div>
          )}
        </div>
      )}
